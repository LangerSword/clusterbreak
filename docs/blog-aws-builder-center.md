...
**Clusterbreak is an interactive 3D simulator for local AI inference rigs, and a real AWS deploy kit for the cloud version of the same rig.** You build a machine from real hardware, run a model, watch it degrade, break it on purpose — then take that rig to your own AWS account as a real `g5.xlarge` and talk to the model running on it.

> **Live:** https://clusterbreak.langersword.in · **Code:** https://github.com/LangerSword/clusterbreak
...
*Will my machine actually run this model?* The honest answer is scattered across benchmark repos, spreadsheets and vibes. Clusterbreak makes the physics visible: you place devices on a 3D board, wire the nodes together, pick a model and a quantization, and run it.
...
Where there is no measurement, the UI marks it with a `~` and says *"no published measurement"* instead of dressing a default up as data.

---

## Then you break it

Unplug a node mid-generation, cut the interconnect, cap VRAM. The postmortem doesn't just say *failed* — it traces the failure back to the exact constraint that produced it: memory, bandwidth, interconnect or compute. The report is a shareable link.

![The postmortem](https://clusterbreak.langersword.in/shots/postmortem.png)
*What died, why, and what to change — with the causal chain, not just a red box.*

---

## Taking the same rig to AWS

From the same board, Clusterbreak generates a **real CloudFormation template** for NVIDIA cards: one node per GPU, launched from the **Deep Learning base AMI** so CUDA is already installed, llama.cpp in Docker behind a per-stack API key, model pulled from Hugging Face in userdata.

### The connect pattern (the part I care most about)
...
```bash
# what the app does, on your behalf, after you install the connect stack
aws sts assume-role \
...
The role is **read-wide, write-narrow**: it can describe stack state, but it can only create and delete `clusterbreak-*` stacks. It worked exactly as documented the first time — which is the highest compliment I can pay an auth flow.

Once connected, the app provisions the rig you simulated, lists your running rigs from the account itself, and lets you **prompt the model on your own instance** from the browser. Tear it down when you're done, and a scheduled sweep kills anything you forget.
...
- **The connect role needed permissions I didn't expect.** The paginated stack listing needs `cloudformation:ListStacks` — a different action from `DescribeStacks` — and `ec2:DescribeKeyPairs` isn't implied by anything else.

### A model that knows where it lives

Ask a quantized LLM what GPU it's on and it will confidently invent one — an 8B told me **A100** while running on an **A10G**. The chat proxy now prepends the model's real deployment facts (region, instance type, GPU, weights file, context window), read from the CloudFormation stack itself. Ask it now:

> *"I am running on a g5.xlarge instance type with an NVIDIA GPU. I am running on an NVIDIA A10G with 24 GB of VRAM."*
...
```bash
# lint the real artifact, not the source
aws cloudformation get-template --stack-name clusterbreak-demo --query TemplateBody > t.json
cfn-lint t.json
```

Linting the artifact caught two bugs that would each have burned a ten-minute GPU deploy:
...
The backend is Python on **boto3** (STS, CloudFormation, EC2, DynamoDB), and the whole build/verify loop runs on **AWS CLI v2**. Services used: **S3, CloudFront, ACM, API Gateway, Lambda, DynamoDB, CloudFormation, IAM, STS, EC2 (`g5.xlarge`), EventBridge, Systems Manager, CloudWatch**.

---

## Try it

1. **Simulate** — https://clusterbreak.langersword.in/app.html — build a rig, break it, share the report.
